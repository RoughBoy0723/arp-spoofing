...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < cnt; i++) {
        ...
        relay_packets(handle);  // 패킷 릴레이 함수 호출
    }
    ...
    while (true) {
        ...
        if (res == 1) {
            ...
            if (eth_hdr->type() == EthHdr::Ip4) {
                struct iphdr* ip_hdr = (struct iphdr*)(packet + sizeof(EthHdr));

                string dest_ip = to_string(ip_hdr->daddr);

                if (IP_MAC.count(dest_ip)) {
                    string real_mac = IP_MAC[dest_ip];
                    memcpy(eth_hdr->dmac_, Mac(real_mac).operator uint8_t*(), Mac::SIZE);
